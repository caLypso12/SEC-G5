...
void HWSW_conv2D(const unsigned char *matrix_in, unsigned char *matrix_out) {
     ...
     * ===================================================================================== */
        ...
        volatile int *image_in_hw = (int *)(CONV2D_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_IMAGE_IN_DATA);
        volatile int *image_out_hw = (int *)(CONV2D_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_IMAGE_OUT_DATA);
        volatile int *weights = (int *)(CONV2D_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_WEIGHTS_DATA);
        volatile int *bias_hw = (int *)(CONV2D_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_BIAS_DATA);
        volatile int *do_conv2d = (int *)(CONV2D_BASEADDR + XAXIL_CONV2D_BUS1_ADDR_AP_CTRL);
       // XTime tStart, tEnd, time_s;

        memcpy(image_in_hw, matrix_in, IMAGE_HEIGHT * IMAGE_WIDTH );  // Copy Image_in Vector to HW
        memcpy(weights, kernel, KERNEL_SIZE * KERNEL_SIZE );       // Copy Kernel Vector to HW
       
        *bias_hw = bias;                                            // Inicialize Bias registor
        //XTime_GetTime(&tStart);
        *do_conv2d = 1;

                                                      //Initiate Calculations
        while((*do_conv2d & 0x2) != 0x2);                               // Wait for result
        //XTime_GetTime(&tEnd);
        //time_s = tStart -  tEnd;
        //printf("#IP Processing Only (No Overhead): %.2f ms.\n\r", (float) time_s * 1000 / (COUNTS_PER_SECOND));
        memcpy(matrix_out, image_out_hw, OUTPUT_HEIGHT * OUTPUT_WIDTH );   //Copy the result out of HW
...
int main() {
     ...
     * ===================================================================================== */
    hw_image_out = (unsigned char *) MEM_DATA_HW_BASE_ADDRESS;
  ...
  printf("# SW Execution: %.2f ms.\n\r", (float) tSW * 1000 / (COUNTS_PER_SECOND));
   ...
   printf("# Speedup: %.2f.\n\r", (float) tSW / tHW);
    ...
    print_ppm(hw_image_out, OUTPUT_HEIGHT, OUTPUT_WIDTH, "output", IMAGE_TO_CONVOLVE);
